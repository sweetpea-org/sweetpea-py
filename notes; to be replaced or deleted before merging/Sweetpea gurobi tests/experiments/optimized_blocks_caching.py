...
def check_combination(combo):
    try:
        ...
        cong = m.addMVar(shape=(4), vtype=GRB.BINARY, name="cong")
        ...
        for i in range(0, 4):
            ...
            m.addConstr(cong[i] == 0, name="cong"+str(i))

            if i > 0:
                m.addConstr(color[i-1][0] + text[i-1][0] + color[i][1] + text[i][1] >= 4*cong[i], name="congruency"+str(i))
                m.addConstr(cong[i] + 3 >= color[i-1][0] + text[i-1][0] + color[i][1] + text[i][1], name="congruency"+str(i))
